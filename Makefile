...
update-template: ## Update the lambda function code in template.yaml with main.py content
	...
	@awk 'BEGIN {in_code_section=0;} /^ *Code:$$/ {print; getline; print "        ZipFile: |"; getline; while ((getline line < "code_without_main.txt") > 0) print "          " line; in_code_section=1; next} in_code_section && /^  [a-zA-Z]/ {in_code_section=0} {if (!in_code_section) print}' template.yaml > temp.yaml
